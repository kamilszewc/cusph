...
void modelWcsphStandard(int NOB, int TPB, thrust::device_vector<Particle>& pVector, Particle *dPSorted, ParticleBasic *pBackup, uint *gridParticleHash, uint *gridParticleIndex, uint *cellStart, uint *cellEnd, Parameters *par, Parameters *parHost, real time);
void modelWcsphColagrossiLandrini(int NOB, int TPB, thrust::device_vector<Particle>& pVector, Particle *dPSorted, ParticleBasic *pBackup, uint *gridParticleHash, uint *gridParticleIndex, uint *cellStart, uint *cellEnd, Parameters *par, Parameters *parHost, real time);
...
void modelWcsphSzewcOlejnik(int NOB, int TPB, thrust::device_vector<Particle>& pVector, Particle *dPSorted, ParticleBasic *pBackup, uint *gridParticleHash, uint *gridParticleIndex, uint *cellStart, uint *cellEnd, thrust::device_vector<ParticleDispersedPhase>& pDispersedPhaseVector, Parameters *par, Parameters *parHost, real time);

int main(int argc, char *argv[])
{
	STARTLOG("logs/sph.log");
	License license;
	License::Version = "20.10.2015";
	...
	switch (argc) {
	case 1:
		domain = new Domain(CASE, 1.5625, 16);
		...
		if (T_PHASE_OUT > 0)
		{
			//domain->WritePhaseDataToRawFile(0);
			//domain->WritePhaseDataToRawFile(1);
			//domain->WritePhaseDataToRawFile(2);
			domain->WritePhasesToRawFile();
		}
		break;
	case 2:
	...
	HANDLE_CUDA_ERROR( cudaMalloc((void**)&pBackupDevice, parHost->N*sizeof(ParticleBasic)) );
	...
	while (*t < parHost->END_TIME) 
	{
		...
		switch (MODEL){
		case 0:
			modelWcsphStandard(NOB, TPB, pDevice, pSortDevice, pBackupDevice, gridParticleHashDevice, gridParticleIndexDevice, cellStartDevice, cellEndDevice, parDevice, parHost, *t);
			break;
		case 1:
			modelWcsphColagrossiLandrini(NOB, TPB, pDevice, pSortDevice, pBackupDevice, gridParticleHashDevice, gridParticleIndexDevice, cellStartDevice, cellEndDevice, parDevice, parHost, *t);
			break;
		case 2:
			...
			break;
		case 3:
			modelWcsphSzewcOlejnik(NOB, TPB, pDevice, pSortDevice, pBackupDevice, gridParticleHashDevice, gridParticleIndexDevice, cellStartDevice, cellEndDevice, pDispersedPhaseDevice, parDevice, parHost, *t);
			break;
		default:
		...
		if (((*t - timeOfLastPrintOut + 0.5*parHost->DT)/parHost->INTERVAL_TIME) >= 1.0)
		{
			...
			try
			{
				...
				////std::cout << pDispersedPhaseHost->at(0).pos.z << " " << pDispersedPhaseHost->at(0).vel.z << " " << pDispersedPhaseHost->at(0).velFl.z << "\n";
				//std::ofstream file("results/dispersed.dat", std::ios::app);
				//if (file.is_open())
				//{
				//	for (int i = 0; i < parHost->N; i++)
				//	{
				//		if (pHost->at(i).id == 7818)
				//		{
				//			file << *t << " " << -pHost->at(i).vel.z << std::endl;
				//		}
				//	}
				//	//file << *t << " " << -pDispersedPhaseHost->at(0).vel.z << std::endl;
				//	file.close();
				//}
