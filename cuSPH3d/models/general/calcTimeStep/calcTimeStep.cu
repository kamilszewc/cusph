...
struct compareVelocity
{
	...
	{
		real vel_1 = sqrt(pow2(p1.vel.x) + pow2(p1.vel.y) + pow2(p1.vel.z)) + p1.s;
		real vel_2 = sqrt(pow2(p2.vel.x) + pow2(p2.vel.y) + pow2(p2.vel.z)) + p2.s;
...
void calcTimeStep(thrust::device_vector<Particle>& p, Parameters *par, Parameters *parHost)
{
	if (parHost->T_TIME_STEP == 1)
	{
		...
		real maxVelocity = sqrt(pow2(pMaxVelocity[0].vel.x) + pow2(pMaxVelocity[0].vel.y) + pow2(pMaxVelocity[0].vel.z)) + pMaxVelocity[0].s;
		real maxViscosity = pMaxViscosity[0].mi;

		real timeStepVelocity = 0.04 * 0.25f * parHost->H / maxVelocity;
		real timeStepViscosity = 0.04 * 0.125f * pow2(parHost->H) / maxViscosity;
