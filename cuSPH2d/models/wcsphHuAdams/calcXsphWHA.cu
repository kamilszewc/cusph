...
__device__ static real3 interaction(uint i, uint j, real2 dpos, real2 dvel, Particle *p, Parameters *par)
{
	...
	if (q < 2.0) {
		...
		switch (par->T_XSPH) {
		case 1:	return MAKE_REAL3(p[j].m*p[j].vel.x*g / p[j].d, p[j].m*p[j].vel.y*g / p[j].d, g*p[j].m / p[j].d);
		...
		case 3: return MAKE_REAL3(p[j].m*p[j].vel.x*g, p[j].m*p[j].vel.y*g, g*p[j].m);
		default:
			return MAKE_REAL3(0.0, 0.0, 0.0);
		}
