...
void read_parameters_from_xml(const char *filename, Parameters *par)
{
	...
	for (XMLElement* xmlElement = xmlElementParameters->FirstChildElement("parameter"); xmlElement != NULL; xmlElement = xmlElement->NextSiblingElement("parameter"))
	{
		...
		else if (!strcmp(name, "T_STRAIN_TENSOR"))
		{
			par->T_STRAIN_TENSOR = atoi(xmlElement->GetText());
		}
		else if (!strcmp(name, "T_SMOOTHING_DENSITY"))
		{
			par->T_SMOOTHING_DENSITY = atoi(xmlElement->GetText());
		}
		else if (!strcmp(name, "T_VARIABLE_H"))
		{
			par->T_VARIABLE_H = atoi(xmlElement->GetText());
		}
		else if (!strcmp(name, "T_DISPERSED_PHASE_FLUID"))
		{
			par->T_DISPERSED_PHASE_FLUID = atoi(xmlElement->GetText());
		}
		else if (!strcmp(name, "N_DISPERSED_PHASE_FLUID"))
		{
			par->N_DISPERSED_PHASE_FLUID = atoi(xmlElement->GetText());
		}
	...
	par->NC = par->NXC * par->NYC;
	par->I_H = 1.0 / par->H;
	par->DH = 0.01f * par->H;
	//par->KNORM = (real)(M_1_PI * pow2(par->I_H));
	//par->GKNORM = (real)(M_1_PI * pow4(par->I_H));
...
void read_particles_from_xml_file(const char* filename, std::vector<Particle>& p, Parameters* par)
{
	...
	for (XMLElement* xmlElement = xmlElementParticles->FirstChildElement("field"); xmlElement != NULL; xmlElement = xmlElement->NextSiblingElement("field"))
	{
		...
		if (!strcmp(name, "smoothing-length"))
		{
			for (unsigned int i = 0; i < splitedStringValues.size(); i++) p[i].h = atof(splitedStringValues[i]);
		}
